...
from server.schemas import article_schema, video_schema
...
def increment_view(db: Session, id: int):
    ...
    db.query(model.Article).filter(model.Article.id == id).update({'view': ss.view + 1})
    db.commit()
...
def get_accound(db: Session, email: str, password: str):
    stmt = select(model.Account).filter(and_(model.Account.email == email, model.Account.hashed_password == password))
    return db.execute(stmt).scalars().first()
